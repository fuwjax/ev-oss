...
import static org.fuwjax.oss.util.Objects2.nullIf;
...
import org.fuwjax.oss.util.collection.MapDecorator;
...
public interface Wonton {
	...
	public static Object naturalValue(final Wonton w) {
	...
	public static List<? extends Wonton> relaxedArray(final Wonton w) {
		if(w instanceof WArray) {
			return ARRAY.get(w);
		}
	...
	public static Wonton first(final Wonton w) {
		if(w == null) {
			return NULL;
		}
		if(w instanceof WArray) {
	...
	public interface WVoid extends Wonton {
		...
		@Override
		default <T> T as(final Type<T> type) throws ClassCastException {
			return null;
		}
	}
	...
	public interface WArray extends Wonton {
		...
		@Override
		default Wonton get(final Path path) throws NoSuchPathException {
			try {
		...
		default Wonton get(final int index) throws IndexOutOfBoundsException {
			return asArray().get(index);
		}

		@Override
		default void accept(final Path root, final Visitor visitor) {
			Wonton.super.accept(root, visitor);
			int index = 0;
			for(final Wonton wonton : asArray()) {
	...
	public interface WStruct extends Wonton {
		...
		default Wonton get(final Path path) {
			return path.isEmpty() ? this : nullIf(asStruct().get(path.key()), w -> w.get(path.tail()), () -> {
	 ...
	 * @implNote Implementations should override this method when using custom
	 ...
	 * @implNote Implementations should override this method when using custom
	 *           Path implementations.
	 * @param visitor the visitor which will visit this wonton and all nested
	 *           sub-wontons
	 * @throws NoSuchPathException if the path does not exist for this wonton
	 */
	...
	/**
	 * Accepts the visitor for itself and all nested sub-wontons.
	 * @param root path to this wonton
	 * @implNote Container implementations should override this method.
	 * @param visitor the visitor which will visit this wonton and all nested
	 *           sub-wontons
	 * @throws NoSuchPathException if the path does not exist for this wonton
